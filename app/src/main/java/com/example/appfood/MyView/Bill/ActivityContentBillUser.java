...
public class ActivityContentBillUser extends AppCompatActivity implements InterGioHangView, InterHoaDonView {
    ...
    // Thiết lập dữ liệu chi tiết cho hóa đơn
    private void Init() {
    ...
    private void DiaLogUpDate() {
        ...
        spiner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if(position>0){
                    if(hoaDon.getType() <3){
                        presenterHoaDon.CapNhatTrangThai(4, hoaDon.getId());
                        dialog.cancel();
                    }else if(hoaDon.getType() == 4){
                        Toast.makeText(ActivityContentBillUser.this, "Đơn hàng đã hủy!", Toast.LENGTH_SHORT).show();
                    }else{
                        Toast.makeText(ActivityContentBillUser.this, "Đơn hàng bạn không thể hủy", Toast.LENGTH_SHORT).show();
                    }
