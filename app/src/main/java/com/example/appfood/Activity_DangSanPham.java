...
import com.example.appfood.MyView.Admin.ActivityAddProduct;
...
public class Activity_DangSanPham extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (sanPham != null) {
            edtBh.setText(sanPham.getTrongluong());
            edtMt.setText(sanPham.getMota());
            edtNsx.setText(sanPham.getHansudung());
            edtSl.setText(sanPham.getSoluong() + "");
            edtTien.setText(sanPham.getGiatien() + "");
            edtTenSp.setText(sanPham.getTensp());
            edtType.setText(sanPham.getType()+"");
            btnEdit.setVisibility(View.VISIBLE);
            btnDel.setVisibility(View.VISIBLE);
            if (!TextUtils.isEmpty(sanPham.getHinhanh())) {
                Picasso.get().load(sanPham.getHinhanh()).into(imageView);
                image = sanPham.getHinhanh();
            }
        }
        db.collection("LoaiSP").get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            ...
            public void onSuccess(@NonNull QuerySnapshot queryDocumentSnapshots) {
                ...
                ArrayAdapter arrayAdapter = new ArrayAdapter(Activity_DangSanPham.this, android.R.layout.simple_list_item_1, list);
                spinerthongke.setAdapter(arrayAdapter);
                if (list.size() > 0) {
                    spinerthongke.setSelection(1);
                    if (sanPham != null) {
                        for (int i = 0; i < list.size(); i++) {
                            if (list.get(i).equals(sanPham.getLoaisp())) {
    ...
    private void initView() {
        ...
        findViewById(R.id.btn_add_back).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(Activity_DangSanPham.this, ActivityAddProduct.class));
            }
        });
            ...
            try {
                ...
                db.collection("SanPham").add(sp).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                    ...
                    public void onSuccess(@NonNull DocumentReference documentReference) {
                        Toast.makeText(Activity_DangSanPham.this, "Thành công!!!", Toast.LENGTH_SHORT).show();
                        setResult(RESULT_OK);
                        finish();
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(Activity_DangSanPham.this, "Thất bại!!!", Toast.LENGTH_SHORT).show();
                    }
                });
            } catch (Exception e) {
                e.printStackTrace();
        ...
        btnDel.setOnClickListener(view -> {
            db.collection("SanPham").document(sanPham.getId()).delete().addOnSuccessListener(unused -> {
                Toast.makeText(Activity_DangSanPham.this, "Xoá sản phẩm thành công!!!", Toast.LENGTH_SHORT).show();
                setResult(RESULT_OK);
                finish();
            }).addOnFailureListener(e -> {
                Toast.makeText(Activity_DangSanPham.this, "Xoá sản phẩm thất bại!!!", Toast.LENGTH_SHORT).show();
            });
        ...
        btnEdit.setOnClickListener(view -> {
            ...
            try {
                ...
                sp.setLoaisp(spinerthongke.getSelectedItem().toString());
                sp.setHinhanh(image);
                db.collection("SanPham").document(sanPham.getId()).set(sp)
                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                            @Override
                            public void onSuccess(@NonNull Void unused) {
                                Toast.makeText(Activity_DangSanPham.this, "Cập nhật sản phẩm thành công!!!", Toast.LENGTH_SHORT).show();
                                setResult(RESULT_OK);
                                finish();
                            }
                        }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(Activity_DangSanPham.this, "Cập nhật sản phẩm thất bại!!!", Toast.LENGTH_SHORT).show();
                    }
                });
            } catch (Exception e) {
                e.printStackTrace();
