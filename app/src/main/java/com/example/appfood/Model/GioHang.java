...
import com.example.appfood.MyObject.InterGioHang;
...
public class GioHang {
    ...
    private InterGioHang callback;
    private FirebaseFirestore db;
    public GioHang(){
    }
    public GioHang(String id, String id_sanpham, long soluong) {
        this.id = id;
        this.id_sanpham = id_sanpham;
        this.soluong = soluong;
    }
    public GioHang(String id_sanpham, long soluong) {
        this.id_sanpham = id_sanpham;
        this.soluong = soluong;
    }
    public GioHang(InterGioHang callback){
        ...
        db.collection("GioHang").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
            ...
            public void onSuccess(@NonNull QuerySnapshot queryDocumentSnapshots) {
                ...
                if(queryDocumentSnapshots.size()!=0){
                    ...
                    GioHang hangModels = new GioHang(idsp,1);
                    db.collection("GioHang").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
    ...
    public void HandleThanhToan(String ngaydat, String diachi, String hoten, String sdt, String phuongthuc, long tongtien, ArrayList<SanPham> arrayList) {
            ...
            public void onComplete(@NonNull Task<DocumentReference> task) {
                if(task.isSuccessful()){
                    for(SanPham sanPham : arrayList){
                        ...
                        map_chitiet.put("id_sanpham", sanPham.getIdsp());
                        map_chitiet.put("soluong", sanPham.getSoluong());
                        ...
                        db.collection("ChitietHoaDon").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
                            ...
                            public void onComplete(@NonNull Task<DocumentReference> task) {
                                if(task.isSuccessful()){
                                    db.collection("GioHang").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
                                            .collection("ALL").document(sanPham.getId()).delete().addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if(task.isSuccessful()){
                                                callback.OnSucess();
